...
def consolidated_labor_shares(year_result, df_ls_year, use_leontief=False):
    ...
    if use_leontief:
        # 1. ℓ_j = CE_j / X_j — labour coefficient per unit of gross output.
        #    CE is at parent level; subs within a parent share the same ratio
        #    (proportional allocation cancels when dividing by X_parent_total).
        x_parent_total = {}
        for s in subs:
        ...
        #    Use total-use Z (domestic + imported intermediates) so that
        #    col_sum(A_total) = (X − GVA) / X and v'·L = 1 by accounting identity.
        #    This means ℓ'·L is already a value-added labour share — no further
        #    normalisation needed.
        Z_for_leontief = year_result.get('Z_total', Z)
        x_safe = X.replace(0, np.nan)
        A = Z_for_leontief.div(x_safe, axis=1).fillna(0).values
