...
│   ├── capinc_single.py        # CapIncModel_single — two-sector GE model (steady-state,
│   │                           #   perfect-foresight transition path, calibration)
...
│   ├── investment_shares.py    # IO analysis from NAIO1F (DST API); per-year cache management
...
│
├── 0_intermediate/             # Cached intermediate outputs (pickles and parquets)
...
| Step | Notebook | Notes |
|------|----------|-------|
| 1 | `1_qp_mechanism.ipynb` | Model mechanism diagram |
| 2 | `2_labour_shares.ipynb` | Fetches DST data; cached after first run |
| 3 | `2_gross_investment.ipynb` | Investment composition from NAIO1F |
| 4 | `2_phi_argument.ipynb` | Wage/employment evidence for declining φ |
| 5 | `2_gini_v_itan.ipynb` | Distributional correlates |
| 6 | `3_IRF.ipynb` | Baseline IRF simulation |
| 7 | `3_phi_sweep.ipynb` | Sensitivity sweep |

### Caching

IO computations (NAIO1F) are slow on first run. Subsequent runs load from `0_intermediate/`. Use the `load_or_compute_*` wrappers:
...
| NABP36 | GVA and compensation of employees by industry | Labour shares |
| NABB36 | Hours worked and number of employees by industry | Wage and employment ratios |
| NAN1 | Gross domestic product, current prices | GDP normalisation in investment shares |
...
- **Two sectors:** consumption (C) and investment (I), with Cobb-Douglas production and potentially different factor intensities (α, β)
